...
Put the directory under git revision control:
 git init If you type ll (I’ll assume that ll is an alias for ls -alF), you will
...
reports that the file is "Changed but not updated." This means thatthe file has been modified since the last commit, but it is still not
...
staged yet, you can do:
 git checkout -- filenameIf the file has been staged, you must first unstage it:
...
Of course you can add -p and --color to see the full diff in color:
 git log -p --color origin..Let’s make one more modification. Fix the printf, and perhaps change
...
retrieve it and take a look. You do that by "pulling" the changes inmy repository into your repository. Let’s pull the changes we just
